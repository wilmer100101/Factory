...
public class ItemToolRegistry extends ItemRegistry {
    ...
    private Optional<Item> getComponentItemStack(Object key, ConfigurationNode value) {
        ...
        if (!(key instanceof String toolId)) {
            plugin.getComponentLogger().error("Could not get toolId, while fetching tool items");
            return Optional.empty();
        }

        itemStack.editPersistentDataContainer(pdc -> pdc.set(plugin.getComponentManager().getTypeKey(), PersistentDataType.STRING, toolId));

        return Optional.of(new Item(itemStack, new NamespacedKey(plugin, toolId)));
    }
}
